...
{
	float3 materialColor = colorTex.Sample(texSampler, input.texCoord).xyz;
    return CalcLightinig(input.wPos, normalize(input.wNormal), normalize(input.viewDirection), materialColor);
}
